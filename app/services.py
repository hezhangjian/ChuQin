...
class AgentService:
    ...
    def _run_skill_job(self, job_id: str, skill: SkillRecord, file_path: Path) -> None:
        try:
            ...
            logger.exception("Skill run failed for job %s", job_id)
            self._update_job(
                job_id,
                status="error",
                stage="error",
...
class ApiBridge:
    ...
    def chat(self, payload: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            logger.exception("ApiBridge.chat failed")
    ...
    def chatMessage(self, payload: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            logger.exception("ApiBridge.chatMessage failed")
            return {"error": str(exc)}
    ...
    def analyze_csv(self, payload: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            logger.exception("ApiBridge.analyze_csv failed")
            return {"error": str(exc)}
    ...
    def start_skill_run(self, payload: dict[str, Any]) -> dict[str, Any]:
        try:
            ...
            logger.exception("ApiBridge.start_skill_run failed")
            return {"error": str(exc)}
